...
export const SwitchUnits: React.FC<React.PropsWithChildren> = () => {
  const CAKE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"BTAF" | "USD">("BTAF");
  const conversionUnit = editingUnit === "BTAF" ? "USD" : "BTAF";
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "BTAF" ? "USD" : "BTAF";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "BTAF") {
      ...
      setValues({
        BTAF: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      });
    }
  };
